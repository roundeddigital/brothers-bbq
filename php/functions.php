...
// Total items ordered is equal to total gift cards
// meaning that there is no products being ordered
// and there is no reason to charge processing and handling
if ($total_items == $total_item_cards) {
	$fee = 0.00;
} else {
  // card processing surcharge fee
  $percentage = 0.04;
  $surcharge = ( $woocommerce->cart->cart_contents_total + $woocommerce->cart->shipping_total ) * $percentage;
  $woocommerce->cart->add_fee( 'Processing Fee', $surcharge, true, '' );

	// handling surchange fee
	$fee = 13.00;
  $county = array('US');
