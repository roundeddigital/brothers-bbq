...
function uncode_user_save_meta_featured_image( $user_id ) {
    ...
    update_usermeta( $user_id, 'user_uncode_meta', $_POST['user_uncode_meta'] );
}
...
add_action( 'custom_menu_order', 'uncode_change_menu_cap' );
...
function uncode_change_menu_cap( $menu_ord ) {
    ...
    if ( !isset( $submenu['uncode-menu'] ) )
	    return $menu_ord;

    foreach ($submenu['uncode-menu'] as $position => $menu) {
        if ( isset($menu[2]) && $menu[2]=='backups' ) {
            $backups = $menu;
            unset($submenu['uncode-menu'][$position]);
        }
    ...
    $array_first = array_slice($submenu['uncode-menu'], 0, ($options));
    $array_second = array_slice($submenu['uncode-menu'], ($options));
    ...
    $submenu['uncode-menu'] = $array_first;
...
function uncode_related_post_submenu() {
	if ( is_plugin_active( 'related-posts-for-wp/related-posts-for-wp.php' ) && class_exists('RP4WP_Hook_Settings_Page') ) {
		...
		add_submenu_page( 'uncode-menu', 'UNCODE', esc_html__( 'Related Posts', 'related-posts-for-wp' ), 'manage_options', 'rp4wp', array(
			'Uncode_RP4WP_Hook_Settings_Page',
			'screen'
		), 'uncode-menu', 'uncode_welcome_page' );
